...
enum ExterMethod { R, JC, FM };

//Struct data structure for the confusion matrix for the external validation methods
...
// Global variables
string file_name;
int num_cluster, max_iter, num_runs, num_points, dimension, num_true_clusters;
double threshold;
...
void ReadFile(const string &file_name)
{
    ...
    file >> num_points >> dimension_plus_one >> num_cluster;
    dimension = dimension_plus_one - 1;

    data.resize(num_points, vector<double>(dimension));
    true_labels.resize(num_points);
...
//Function to compute the Pariwise Measures Confusion Matrix
ConfusionMatrix ComputePairwiseConfusion(const vector<int>& assignments)
{
    ConfusionMatrix cm;
    ...
    for (int i = 0; i < num_points; i++)
    {
        for (int j = i + 1; j < num_points; j++)
        {
            bool same_cluster_pred = assignments[i] == assignments[j];
            bool same_cluster_true = true_labels[i] == true_labels[j];

            if (same_cluster_pred && same_cluster_true)
            {
                cm.TP++;
            }
            else if (!same_cluster_pred && !same_cluster_true)
            {
                cm.TN++;
            }
            else if (same_cluster_pred && !same_cluster_true)
...
//Function to Calculate the Rand Index Score
double RStatistic(const ConfusionMatrix& cm)
{
    ...
    double score = static_cast<double>(cm.TP + cm.TN) / total;

    return score;
}

//Function to Calculate the Jaccard Coefficient Index Score
double JCoefficient(const ConfusionMatrix& cm)
{
    double denominator = cm.TP + cm.FP + cm.FN;
    if (denominator == 0) 
    {
        return 0.0;
    }
    double score = static_cast<double>(cm.TP) / denominator;

    return score;
}

//Function to Calculate the Fowlkes-Mallows Index Score
double FMMeasure(const ConfusionMatrix& cm)
{
    double score = 0.0;
    double precision_denominator = cm.TP + cm.FP;
    double recall_denominator = cm.TP + cm.FN;
    ...
    double precision = static_cast<double>(cm.TP) / precision_denominator;
    double recall    = static_cast<double>(cm.TP) / recall_denominator;

    score = sqrt(precision * recall);

    return score;
}

//Function to conduct the external validation methods
double ExternalValidation(const vector<int>& assignments, ExterMethod method)
{
    double score = 0.0;

    //Pairwise Measures
    //Compare assignments with true_labels
    ConfusionMatrix cm = ComputePairwiseConfusion(assignments);
...
void k_means(const vector<vector <double> > &dataset, InitMethod method, ExterMethod valid_method)
{
    int best_run = -1;
    ...
    for(int r = 0; r < num_runs; r++)
    {
        ...
        double score = ExternalValidation(assignments, valid_method);
...
int main(int argc, char *argv[])
{
    ...
    cout << "Validation Method, Best Score, Run #" << endl;
    cout << "Rand Statistic, ";
    k_means(minmax_data, RANDOM_SELECTION, R);

    cout << "Jaccard Coefficient, ";
    k_means(minmax_data, RANDOM_SELECTION, JC);
    
    cout << "Fowlkes–Mallows Measure, ";
    k_means(minmax_data, RANDOM_SELECTION, FM);
